...
namespace net45.Utility
{
    class SM4
    {
        ...
        {
            var n = (long)(b[i] & 0xff) << 24 | (b[i + 1] & 0xff) << 16 | (b[i + 2] & 0xff) << 8 | b[i + 3] & 0xff & 0xffffffffL;
            return n;
        }
        ...
        public void SM4SetKeyEncrypt(SM4Context ctx, byte[] key)
        {
            ctx.Mode = SM4_ENCRYPT;
            sm4_setkey(ctx.Sk, key);
        }

        public void SM4SetKeyDecrypt(SM4Context ctx, byte[] key)
        {
            var i = 0;
